...
\begin{enumerate}
    \item  \texttt{AudioDeviceProvider} ne dépend en rien du reste de l'application, il peut donc être testé indépendamment des autres classes~;
    \item \texttt{InPort}, \texttt{OutPort} et \texttt{VirtualPort} dépendent uniquement de \texttt{Module}, mais cette dépendance sert seulement au parcours du graphe d’objets. Le comportement des ports peut donc être testé avant \texttt{Module}~;
    ...
    \item Le \texttt{Sequencer} sera intégré et testé grâce à des \textit{mocks} simulant le comportement de modules~;
