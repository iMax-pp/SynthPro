...
bool Port::connect(Port* other)
{
    if (vPort()->connectable(other->vPort())) {
        ...
        qDebug(QString("VirtualPort::connectTo() %1 -> %2").arg(vPort()->name()).arg(other->vPort()->name()).toLatin1());
...
{
    if (m_connection) {
        qDebug(QString("VirtualPort::disconnectFrom() %1 - %2").arg(vPort()->name()).arg(m_connection->vPort()->name()).toLatin1());
